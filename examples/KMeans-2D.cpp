...
plt.scatter(
       ...
       X.slice(
           :, 1),
       c = 'white', marker = 'o', edgecolor = 'black', s = 50) plt.show()

    // create KMeans object
    Matrix km = KMeans(n_clusters = 3, init = 'random', n_init = 10, max_iter = 300, tol = 1e-04,
                       random_state = 0)

    Matrix Y_pred =
        km.fit_predict(X) // fit() and predict()
        ...
        plt.scatter(
            ...
            plt.scatter(X.slice(Y_pred == 1, 0), X.slice(Y_pred == 1, 1), s = 50, c = 'orange',
                        marker = 'o', edgecolor = 'black', label = 'cluster 2')

                plt.scatter(X.slice(Y_pred == 2, 0), X.slice(Y_pred == 2, 1), s = 50,
                            c = 'lightblue', marker = 'v', edgecolor = 'black', label = 'cluster 3')

        // also plot the centroids of the categorized data
        plt.scatter(km.cluster_centers().slice(
                        :, 0),
                    km.cluster_centers().slice(
                        :, 1),
                    s = 250, marker = '*', c = 'red', edgecolor = 'black',
                    label = 'centroids') plt.legend(scatterpoints = 1) plt.grid() plt.show()